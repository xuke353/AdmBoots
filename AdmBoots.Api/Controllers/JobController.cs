...
namespace AdmBoots.Api.Controllers {
    ...
    public class JobController : ControllerBase {
        ...
        /// <returns></returns>
        [HttpGet("logs")]
        public async Task<IActionResult> GetJobLog([FromQuery]JobKeyInput input) {
            var result = await _schedulerCenter.GetJobLogsAsync(input.JobName, input.GroupName);
            return Ok(ResponseBody.From(result));
        }
        /// <summary>
        /// 添加任务
        ...
        [HttpPut("pause")]
        public async Task<IActionResult> Pause([FromBody]AddScheduleInput input) {
        ...
        /// <returns></returns>
        [HttpPut("pesume")]
        public async Task<IActionResult> Resume([FromBody]AddScheduleInput input) {
            await _schedulerCenter.ResumeJobAsync(input.JobName, input.GroupName);
            return Ok(ResponseBody.From("启动成功"));
        }
        [HttpPut("run")]
        public async Task<IActionResult> Run([FromBody]AddScheduleInput input) {
